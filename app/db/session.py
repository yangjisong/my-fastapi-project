...
# --- 2. 生命周期钩子：初始化与关闭 ---
asyncdefsetup_database_connection():
    """在应用启动时，初始化全局的数据库引擎和会话工厂。"""
    global _engine, _SessionFactory
    if _engine isnotNone:
...
asyncdefclose_database_connection():
...
asyncdefget_db() -> AsyncGenerator[AsyncSession, None]:
    ...
    """
    if _SessionFactory isNone:
        raise RuntimeError("数据库会话工厂未初始化！")

    # 从会话工厂中创建一个新的会话
    asyncwith _SessionFactory() as session:
        # 使用 yield 将会话提供给路径函数
        yield session
...
asyncdefcreate_db_and_tables():
    ...
    asyncwith _engine.begin() as conn:
